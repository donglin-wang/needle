...
inline void save_corpus(const std::vector<int32_t> &text, const std::string &path)
{
    ...
    f.write(reinterpret_cast<const char *>(text.data()), n * sizeof(int32_t));
}
...
public:
    ...
    explicit SuffixArrayIndexer(std::vector<int32_t> codepoints)
    {
...
private:
    void build(std::vector<int32_t> text)
    {
        if (text.empty())
            return;
        ...
        int32_t ret = libsais_int(text.data(), sa.data(), static_cast<int32_t>(n), k, 0);
        if (ret != 0)

...
static std::vector<uint32_t> parse(std::initializer_list<uint8_t> bytes)
{
    UTF8Reader reader("");
...
TEST(UTF8Reader, ASCIISingleBytes)
{
    ...
    EXPECT_EQ(result, (std::vector<uint32_t>{'H', 'i', '!'}));
}
...
TEST(UTF8Reader, TwoByteSequence)
{
    ...
    EXPECT_EQ(result, (std::vector<uint32_t>{0x00E9}));
}
...
TEST(UTF8Reader, ThreeByteSequence)
{
    ...
    EXPECT_EQ(result, (std::vector<uint32_t>{0x4E2D}));
}
...
TEST(UTF8Reader, FourByteSequence)
{
    ...
    EXPECT_EQ(result, (std::vector<uint32_t>{0x1F600}));
}

TEST(UTF8Reader, InvalidLeadingByteProducesReplacement)
{
    auto result = parse({0xFF});
    EXPECT_EQ(result, (std::vector<uint32_t>{0xFFFD}));
}
...
TEST(UTF8Reader, MixedASCIIAndMultibyte)
{
    ...
    EXPECT_EQ(result, (std::vector<uint32_t>{'A', 0x00E9, 'Z'}));
}
